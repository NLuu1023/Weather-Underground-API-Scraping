...
public class Credentials {
    ...
    private static boolean populate() {
        ...
        if (url.length() < 2){
            ...
            port = Integer.parseInt(urlNode.attr("port"));
            ...
            Elements userNodes = doc.select("user");
            user = userNodes.attr("UserID");
            password = userNodes.attr("password");

            Elements keyNodes = doc.select("weatherUnderground");
            apiKey = keyNodes.attr("key");

            //TODO Some logic to indicate success or failure.
            if(url.length() > 2)success = true;
        }
        return success;
    }
